  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center space-x-1 px-2 py-1 rounded-md border border-white/20 hover:border-white/40 transition-colors text-white bg-black/20 hover:bg-black/40"
      ...
      {/* Dropdown com opções */}
      {isOpen && (
        <div className="absolute top-full mt-1 right-0 bg-black border border-white/20 rounded-md shadow-lg z-50 min-w-[120px]">
          {availableLocales.map((loc) => {
            ...
            return (
              ...
              >
                <span className="font-medium">{config.name} - {config.fullName}</span>
              </button>
            );
          })}
        </div>
